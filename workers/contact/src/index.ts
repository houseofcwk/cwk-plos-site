// CWK. Contact — standalone Cloudflare Worker
// Mirrors the waitlist worker pattern. Deploys to api.houseofcwk.com/contact.
//
// Endpoints:
//   POST /contact            → submit contact form
//   GET  /contact/healthz    → health check
//
...
function json(body: object, status: number, env: Env, extra: Record<string, string> = {}): Response {
...
async function handleContactPost(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (pathname === '/contact/healthz' || pathname === '/healthz') {
